...
describe('emailService (new)', () => {
  ...
  describe('sendPasswordReset', () => {
    test('calls template and resendClient with reset URL', async () => {
      mockResendClient.sendEmail.mockResolvedValue({ success: true, id: 'email_123' });
      const { sendPasswordReset } = require(MODULE_PATH);
      ...
      expect(mockTemplates.passwordResetEmail).toHaveBeenCalledWith({
        email: 'test@example.com',
        resetUrl: 'https://example.com/reset?token=abc123',
      });
      expect(mockResendClient.sendEmail).toHaveBeenCalledWith(
  ...
  describe('sendUsageSummary', () => {
    test('calls template and resendClient with usage stats', async () => {
      mockResendClient.sendEmail.mockResolvedValue({ success: true, id: 'email_123' });
      const { sendUsageSummary } = require(MODULE_PATH);

      await sendUsageSummary({
        email: 'test@example.com',
        pluginName: 'AltText AI',
        stats: { totalImages: 100, monthlyImages: 50 },
      });

      expect(mockTemplates.usageSummaryEmail).toHaveBeenCalledWith({
        email: 'test@example.com',
        pluginName: 'AltText AI',
        stats: { totalImages: 100, monthlyImages: 50 },
      });
    ...
    test('works without plugin name', async () => {
      mockResendClient.sendEmail.mockResolvedValue({ success: true, id: 'email_123' });
      const { sendUsageSummary } = require(MODULE_PATH);
      ...
      expect(mockResendClient.sendEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          tags: [{ name: 'event', value: 'usage_summary' }],
        })
      );
    });

    test('handles email sending failure', async () => {
      mockResendClient.sendEmail.mockResolvedValue({ success: false, error: 'Invalid email' });
      const { sendUsageSummary } = require(MODULE_PATH);

      const result = await sendUsageSummary({
        email: 'test@example.com',
      });
  ...
  describe('sendMagicLink', () => {
    test('sends magic link email with correct URL', async () => {
      mockResendClient.sendEmail.mockResolvedValue({ success: true, id: 'email_123' });
      const { sendMagicLink } = require(MODULE_PATH);
    ...
    test('returns deduplicated response when recent event exists', async () => {
      mockEmailEventService.hasRecentEvent.mockResolvedValue(true);
      const { sendMagicLink } = require(MODULE_PATH);
