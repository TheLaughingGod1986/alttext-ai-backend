...
async function createLicense(options = {}) {
  ...
  if (error) {
    logger.error('Error creating license', { error: error.message });
    throw new Error(`Failed to create license: ${error.message}`);
  }

  logger.info(`License created`, { licenseKey, plan, service });
  ...
  if (siteUrl || siteHash || installId) {
    try {
      ...
      logger.warn('Auto-attach failed during license creation', { error: attachError.message });
    }
  }
  ...
  if (email) {
    try {
      ...
      logger.warn('License email failed during creation', { error: emailError.message });
    }
  }
...
async function autoAttachLicense(licenseId, siteInfo = {}) {
  ...
  logger.info(`License attached to site`, { licenseKey: license.licenseKey, siteHash: site.siteHash });
